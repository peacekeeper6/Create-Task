...
def no_space_concise_uni(choice): 
  global index # global variable must be initialized to be accessed
...
def no_space_concise_no_uni(choice): 
  global index
  prompt("Here is your username, thanks for playing: ")
  prompt(array_no_space_concise_no_uni[random.randint(0, len(array_no_space_concise_no_uni - 1))]) # has to be len - 1 because the length of the array only represents the number of items, not the array's last item
  time.sleep(0.5)
  while index < 2:
    choice = input("\n" + "Do you want to reroll? Duplicates may occur." + "\n" + "Y/N: ")
    try: 
